...
files_ready(){ [[ -x "$APP_DIR/venv/bin/python" && -f "$APP_DIR/panel.py" && -f "$APP_DIR/kvm.py" && -f "$APP_DIR/bot.py" && -f "$APP_DIR/launcher.py" && -f "$APP_DIR/panel_bridge.py" && -f "$APP_DIR/user_portal.py" && -f "$APP_DIR/requirements.txt" ]]; }
...
install(){
 ...
 mkdir -p "$APP_DIR"
 info "Downloading Snck panel, KVM, bot and customer bridge..."
 for f in panel.py kvm.py bot.py launcher.py panel_bridge.py user_portal.py requirements.txt start.sh; do curl -fsSL "$REPO_RAW/$f" -o "$APP_DIR/$f" || die "Failed to download $f"; done
 ...
 python3 -m py_compile panel.py kvm.py bot.py launcher.py panel_bridge.py user_portal.py || die "Python validation failed."
...
EOF
  ...
  systemctl enable "$SERVICE_NAME" "$PORTAL_SERVICE" "$BOT_SERVICE" >/dev/null
  systemctl start "$SERVICE_NAME" "$PORTAL_SERVICE"
  sleep 2
  ...
  warn "systemd is unavailable; start panel.py and user_portal.py manually."
 fi
 printf '\n%b╭────────────── SNCK INSTALL COMPLETE ──────────────╮%b\n' "$GREEN" "$RESET"
 printf '%b│%b Admin Panel:   http://%s:5000\n' "$GREEN" "$RESET" "$ip"
 printf '%b│%b Customer Panel: http://%s:5001\n' "$GREEN" "$RESET" "$ip"
 printf '%b│%b License: %s\n' "$GREEN" "$RESET" "$license"
 printf '%b│%b Admin user: %s\n' "$GREEN" "$RESET" "$admin"
 printf '%b│%b Admin password: %s\n' "$GREEN" "$RESET" "$password"
 printf '%b│%b Discord Bot: configure from Admin Panel → Discord Bot\n' "$GREEN" "$RESET"
 printf '%b╰────────────────────────────────────────────────────╯%b\n' "$GREEN" "$RESET"
 warn "The Discord bot service is enabled but will remain offline until a token is configured in the panel."
}
update(){
 require_root; installed || { warn "Panel is not installed."; return; }
 info "Updating Snck panel, KVM module, bot and customer bridge..."
 for f in panel.py kvm.py bot.py launcher.py panel_bridge.py user_portal.py requirements.txt start.sh; do curl -fsSL "$REPO_RAW/$f" -o "$APP_DIR/$f" || die "Failed to update $f"; done
 "$APP_DIR/venv/bin/pip" install -r "$APP_DIR/requirements.txt" || die "Dependency update failed."
 python3 -m py_compile "$APP_DIR"/*.py || die "Python validation failed."
 systemctl restart "$SERVICE_NAME" "$PORTAL_SERVICE" 2>/dev/null || true
 if [[ -s "$APP_DIR/.env" ]] && grep -q '^DISCORD_TOKEN=' "$APP_DIR/.env"; then systemctl restart "$BOT_SERVICE" 2>/dev/null || true; fi
 ok "Snck panel, customer portal, KVM module and bot files updated."
}
uninstall(){
 require_root; installed || { warn "Nothing installed."; return; }
 printf 'Remove Snck panel, customer portal and local data? [y/N]: '; IFS= read -r c <&3 || true; [[ $c =~ ^[Yy]$ ]] || return
 systemctl disable --now "$SERVICE_NAME" "$PORTAL_SERVICE" "$BOT_SERVICE" 2>/dev/null || true
 rm -f "/etc/systemd/system/$SERVICE_NAME.service" "/etc/systemd/system/$PORTAL_SERVICE.service" "/etc/systemd/system/$BOT_SERVICE.service"
 systemctl daemon-reload 2>/dev/null || true; rm -rf "$APP_DIR"; ok "Snck panel removed."
}
