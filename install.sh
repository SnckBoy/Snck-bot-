...
APP=${SNCK_APP_DIR:-/opt/snck-bot}; PS=snck-kvm-panel; BS=snck-discord-bot; V=8.2.5
...
uninstall(){ root;printf '\nSNCK PANEL + BOT UNINSTALL\nType UNINSTALL or y to continue: ';local x;read -r x <&3||x=;[[ "$x" = UNINSTALL || "$x" =~ ^[Yy]([Ee][Ss])?$ ]]||{ echo 'Cancelled.';return;};systemctl stop $PS $BS 2>/dev/null||:;systemctl disable $PS $BS 2>/dev/null||:;rm -f /etc/systemd/system/$PS.service /etc/systemd/system/$BS.service /etc/systemd/system/multi-user.target.wants/$PS.service /etc/systemd/system/multi-user.target.wants/$BS.service;systemctl daemon-reload;systemctl reset-failed $PS $BS 2>/dev/null||:;rm -rf "$APP";echo 'SNCK PANEL + BOT UNINSTALLED SUCCESSFULLY';printf 'Press Enter... ';read -r _ <&3||:;}
