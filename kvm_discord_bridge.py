...
def ensure_schema():
 ...
 try:
  ...
  for n,t in {'storage':"TEXT NOT NULL DEFAULT '/var/lib/libvirt/images'",'total_vps':'INTEGER DEFAULT 100','tags':"TEXT DEFAULT '[]'",'api_key':'TEXT'}.items():
...
def node_uri(node): return str((node or {}).get('url') or '')
def ram_gb(v):
 s=str(v).strip().lower().replace(' ','')
 if s.endswith('mb'): return max(1,int(float(s[:-2])/1024))
 if s.endswith('gb'): return int(float(s[:-2]))
 return int(float(s))
def disk_gb(v):
 s=str(v).strip().lower().replace(' ','')
