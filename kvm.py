...
def ensure_default_network(uri=""):
    """Ensure the libvirt default NAT network is defined and active."""
    ...
    if "default" not in networks.splitlines():
        ...
        if not xml.exists():
            raise RuntimeError("Libvirt default network is missing.")
...
def start(n,u=""):
    ensure_default_network(u)
    return virsh(u,["start",n])
...
def create(name,ram,cpu,disk,password,image=DEFAULT_IMAGE,storage="/var/lib/libvirt/images",uri=""):
    ...
    if not tools_ok():
        missing=[x for x in ("virsh","virt-install","cloud-localds","qemu-img") if not shutil.which(x)]
    ...
    if not img.exists(): run(["curl","-fL","--retry","3","--connect-timeout","15","--max-time","900","-o",str(img),image],920)
