...
The panel accepts a bot token, optionally verifies the application/client ID,
saves credentials server-side, and restarts the bot automatically.
"""
...
@admin_required
def bot():
    """Configure and start the Discord bot from the panel.

    A Discord bot token is the credential required for the Gateway connection.
    Client ID and Guild ID are optional metadata; the Client ID is verified
    against Discord when supplied.
    """
    if request.method == 'POST':
        ...
        elif 'DISCORD_GUILD_ID' not in existing:
        ...
        # Restart the bot immediately so saving the form also starts the bot.
        try:
            if os.getenv('SNCK_CODESPACE') == '1':
                ...
                if not python_bin.exists():
                    python_bin = BASE / 'venv' / 'bin' / 'python'
                log = open(BASE / '.codespace' / 'bot.log', 'a') if (BASE / '.codespace').exists() else subprocess.DEVNULL
                subprocess.Popen([str(python_bin), str(BASE / 'launcher.py')],
                                 cwd=str(BASE), stdout=log, stderr=log,
...
<div class="grid"><div class="card stat"><div class="muted">Connection</div><div class="value {status_class}">{bot_status}</div></div><div class="card stat"><div class="muted">Application</div><div class="value">{html.escape(client_id or 'Not set')}</div></div></div>
