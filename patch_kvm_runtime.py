...
async def deploy_cmd(ctx):
    ...
    if getattr(ctx.author, 'bot', False): return
    if DEPLOY_ROLE_ID:
        ...
        if DEPLOY_ROLE_ID not in roles and not getattr(ctx.author.guild_permissions,'administrator',False):
            await ctx.send(embed=create_error_embed('Access Denied','You do not have permission to deploy a VPS.')); return
    handler=getattr(bot,'_snck_kvm_deploy',None)
    if handler is None:
        await ctx.send(embed=create_error_embed('KVM Backend Offline','The KVM integration is not loaded. Restart the Snck Discord bot service.')); return
    try: await handler(ctx,ctx.author)
...
def patch_launcher():
    ...
    text=p.read_text(encoding='utf-8'); marker='mod._do_free_deploy=free_deploy'; binding='mod.bot._snck_kvm_deploy=free_deploy'
...
def patch_panel():
    ...
    # Make the panel's VPS records match the bot's shared schema.
    old_insert="c.execute('INSERT INTO vps(user_id,node_id,name,ram,cpu,disk,status,created_at,root_password) VALUES(?,?,?,?,?,?,?,?,?)',(uid,nid,name,ram,cpu,disk,'running',__import__('datetime').datetime.utcnow().isoformat(),pw)); c.commit()"
    new_insert="c.execute('INSERT INTO vps(user_id,node_id,name,container_name,ram,cpu,disk,storage,config,os_version,status,created_at,root_password) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',(uid,nid,name,name,ram,cpu,disk,f'{disk}GB',f'{ram}MB RAM / {cpu} CPU / {disk}GB Disk',osver,'running',__import__('datetime').datetime.utcnow().isoformat(),pw)); c.commit()"
    if old_insert in text: text=text.replace(old_insert,new_insert,1)
