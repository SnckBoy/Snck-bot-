...
def patch_bot():
    p=ROOT/'bot.py'; text=p.read_text() if p.exists() else (_ for _ in ()).throw(SystemExit('Missing bot.py'))
    text=replace_block(text,r"@bot\.command\(name=['\"]deploy['\"]\).*?(?=\n@bot\.(?:command|event)\b)",BOT_ROUTE,'deploy command in bot.py'); p.write_text(text)
def patch_launcher():
    p=ROOT/'launcher.py'; text=p.read_text() if p.exists() else (_ for _ in ()).throw(SystemExit('Missing launcher.py'))
    ...
    if 'mod.bot._snck_kvm_deploy=free_deploy' not in text: text=text.replace(marker,marker+'\n mod.bot._snck_kvm_deploy=free_deploy',1)
    p.write_text(text)
def patch_panel():
    p=ROOT/'snck_panel.py'; text=p.read_text() if p.exists() else (_ for _ in ()).throw(SystemExit('Missing snck_panel.py'))
...
def patch_panel_login():
    ...
    if old in text: text=text.replace(old,new,1)
    elif "SELECT * FROM panel_users WHERE username=? AND active=1" not in text: raise SystemExit('Could not locate panel customer login handler')
    p.write_text(text)
def main():
    patch_bot(); patch_launcher(); patch_panel(); patch_panel_login(); print('Snck KVM runtime repair applied.')
